...
def call_openai(
    ...
    client = OpenAI(api_key=os.getenv("sk- proj-4kG3CCSKQaGuk5kK5y6rVBWiQ34Vgv1JMNfdhhKh_ssn54 q83xalfmferxeR1x3dLcZvXTE6217381bkFJUhOOtdUdxItiYSa -007pqz7sZ51PZ0wLSv13WgJhc150NUWLeLVIIBSPs9RKYFnXOU RVKJTHAOPENAI_API_KEY"))
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost_estimate = (len(gpt4o_response.split()) /
    0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    return {"input_tokens": input_tokens, "output_tokens": output_tokens, "input_cost": input_cost, "output_cost": output_cost, "total_cost": total_cost}
...
def run_assistant(
    ...
    if get_input is None:
        get_input = input
    from openai import OpenAI
    openai_client = OpenAI(api_key=os.getenv("sk- proj-4kG3CCSKQaGuk5kK5y6rVBWiQ34Vgv1JMNfdhhKh_ssn54 q83xalfmferxeR1x3dLcZvXTE6217381bkFJUhOOtdUdxItiYSa -007pqz7sZ51PZ0wLSv13WgJhc150NUWLeLVIIBSPs9RKYFnXOU RVKJTHAOPENAI_API_KEY"))
    history, num_turns, total_tokens, total_cost = [], 0, 0, 0.0
    while True:
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        stream = retry_with_backoff(lambda: openai_client.chat.completions.create(
...
if __name__ == "__main__":
    ...
    prompt_text = "Hãy kể cho tôi một sự thật thú vị về Việt Nam."
    response, latency = call_openai(prompt=prompt_text, temperature=0.0)
    print("=== KẾT QUẢ VỚI TEMPERATURE 0.0 ===")
    print("Thời gian:", latency, "giây")
    print("Câu trả lời:\n", response)
